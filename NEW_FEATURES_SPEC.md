...
### Доп. правка (2026-09-23): кнопка «Уже оплачено» под напоминанием

Чтобы бот не спамил напоминаниями после того, как платёж уже внесён (человек мог заплатить раньше
срока или сразу после первого напоминания), под каждым сообщением-напоминанием теперь есть инлайн-
кнопка. Тап по ней **гасит все оставшиеся офсеты (3/2/1/0) для этой конкретной даты платежа** — ничего
не пишет ни в Google Sheets, ни в «Факт оплаты» кредитного листа (это по-прежнему делает обычный
expense-флоу с подкатегорией кредита), только подавляет будущие напоминания в БД.

- Новая таблица `credit_payment_ack` (`credit_key`, `payment_date`, `acked_at_utc`) — `willem/db/schema.sql`.
- CRUD: `willem/db/credit_reminders.py::ack_payment`/`acknowledged_payment_dates` (идемпотентно —
  повторный тап не падает на `PRIMARY KEY`).
- `due_reminders` (`willem/credit_reminders.py`) получил третий параметр `acknowledged: set[str]` —
  строки с датой платежа в этом множестве больше не считаются «due», независимо от офсета.
- `send_due_reminders` прикрепляет к каждому напоминанию `InlineKeyboardMarkup` с одной кнопкой
  (`_ack_keyboard`); `callback_data` кодирует индекс кредита в `config.credit_sheets` (тот же приём,
  что у `VIEW_PREFIX`/`TOGGLE_PREFIX` в `willem/bot/handlers/credits.py`) и ISO-дату платежа —
  `credit_ack:{idx}:{payment_date}`.
- Новый обработчик `willem/bot/handlers/credits.py::ack_payment` — по тапу помечает платёж
  оплаченным, отвечает тостом и переиздаёт сообщение без кнопки, дописав подтверждение в текст.
- Важное ограничение: подтверждение глобально гасит напоминания по этой дате **для всех участников
  профиля** (Лика/Ярослав/Админ) сразу — но визуально редактируется только то сообщение, где нажали
  кнопку; копии у остальных получателей (это разные сообщения в разных чатах) остаются с кнопкой,
  просто больше не понадобятся, так как новых напоминаний по этой дате уже не придёт.
- Новые тексты `credits.mark_paid_button`/`marked_paid`/`marked_paid_suffix` в `texts_base.yaml` +
  голос Панталоне в `pantalone.yaml` (без эмодзи/восклицаний, как и остальные тексты профиля).
- Тесты: `tests/test_credit_reminders_db.py` (CRUD + идемпотентность), `tests/test_credit_reminders.py`
  (`due_reminders` игнорирует acknowledged-даты, `send_due_reminders` их не рассылает). Сам callback-
  хендлер `ack_payment` не покрыт unit-тестом — аналогично остальной aiogram-маршрутизации в проекте
  (см. README: «Маршрутизация aiogram… проверяется вручную вживую»), стоит один раз проверить вживую
  после деплоя.
